...
main() {
    ...
    echo ""
    warning "This will completely remove all OpenSearch observability stack components:"
    echo "  • OTEL Demo (namespace: otel-demo)"
    echo "  • Jaeger (namespace: jaeger)"  
    echo "  • OpenSearch Dashboards (namespace: opensearch)"
    echo "  • OpenSearch (namespace: opensearch)"
    echo ""
    ...
    log "You can now re-run the deployment script if needed"
    echo ""
}
